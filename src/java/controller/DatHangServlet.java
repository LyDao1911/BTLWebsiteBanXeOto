...
public class DatHangServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // THÊM: Lấy thông tin phụ phí
        String insuranceFeeStr = request.getParameter("insuranceFee");
        String fixedFeeStr = request.getParameter("fixedFee");
        ...
        // THÊM: TÍNH TOÁN PHỤ PHÍ
        BigDecimal taxRate = new BigDecimal("0.10"); // Thuế 10%
        BigDecimal taxAmount = finalTotalAmount.multiply(taxRate);

        // Phí cố định (mặc định 12 triệu)
        BigDecimal fixedFee = new BigDecimal("12000000");
        if (fixedFeeStr != null && !fixedFeeStr.trim().isEmpty()) {
            try {
                fixedFee = new BigDecimal(fixedFeeStr.trim().replaceAll("[^\\d]", ""));
            } catch (NumberFormatException e) {
                System.out.println("DEBUG: Lỗi định dạng fixedFee, sử dụng mặc định 12,000,000");
            }
        }

        // Bảo hiểm vật chất (3.5% giá trị xe)
        BigDecimal insuranceAmount = BigDecimal.ZERO;
        boolean hasInsurance = "true".equals(insuranceFeeStr);
        if (hasInsurance) {
            BigDecimal insuranceRate = new BigDecimal("0.035"); // 3.5%
            insuranceAmount = finalTotalAmount.multiply(insuranceRate);
        }

        // Tổng phụ phí
        BigDecimal extraFees = fixedFee.add(insuranceAmount);

        // Tổng tiền cuối cùng = Giá sản phẩm + Thuế + Phụ phí
        finalTotalAmount = finalTotalAmount.add(taxAmount).add(extraFees);
        ...
        try {
            // THÊM: Ghi chú chi tiết về phụ phí
            String noteContent = String.format(
                    "PTTT: %s | Người nhận: %s | SĐT: %s | Địa chỉ: %s | Phí trước bạ: %s | Bảo hiểm: %s",
                    paymentMethod, receiverName, receiverPhone, receiverAddress,
                    formatCurrency(fixedFee),
                    hasInsurance ? formatCurrency(insuranceAmount) : "Không"
            );
            ...
            System.out.println("DEBUG: Tổng tiền: " + finalTotalAmount);
            System.out.println("DEBUG: Phí cố định: " + fixedFee);
            System.out.println("DEBUG: Bảo hiểm: " + (hasInsurance ? insuranceAmount : "Không có"));
    ...
    // THÊM: Hàm định dạng tiền tệ cho ghi chú
